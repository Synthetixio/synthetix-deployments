...
async function payDebt({ accountId, amount }) {
  log({ accountId, amount });
  ...
  const tx = await PerpsMarketProxy.connect(signer).payDebt(
    accountId,
  ...
  await tx
    ...
    .then(gasLog({ action: 'PerpsMarketProxy.payDebt', log }));
