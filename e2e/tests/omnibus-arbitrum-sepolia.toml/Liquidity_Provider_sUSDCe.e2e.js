...
describe(require('path').basename(__filename, '.e2e.js'), function () {
  ...
  it(`should increase max pool collateral for the test`, async () => {
    const { tokenAddress } = await getCollateralConfig('fsUSDCe');
      ...
      collateralLimit:
        parseFloat(
          ethers.utils.formatEther(
            require('../../deployments/extras.json').max_collateral_limit_sUSDCe
          )
        ) * 10,
      issuanceRatio: 0,
    });
  ...
  it('should make a price update', async () => {
    ...
    await doPriceUpdateForPyth({
      wallet,
      feedId: require('../../deployments/extras.json').pyth_feed_id_sUSDCe,
      priceVerificationContract: require('../../deployments/extras.json')
        .pyth_price_verification_address,
    });
    await doPriceUpdateForPyth({
  ...
  it('should set fsUSDCe balance to 1000', async () => {
    assert.equal(
      await getCollateralBalance({ address, symbol: 'fsUSDCe' }),
      0,
      'New wallet has 0 fsUSDCe balance'
    );
    const { tokenAddress } = await getCollateralConfig('fsUSDCe');
    await setMintableTokenBalance({ privateKey, tokenAddress, balance: 1000 });
    assert.equal(await getCollateralBalance({ address, symbol: 'fsUSDCe' }), 1000);
  });

  it('should approve fsUSDCe spending for CoreProxy', async () => {
    assert.equal(
      ...
      false,
      'New wallet has not allowed CoreProxy fsUSDCe spending'
    );
  ...
  it('should deposit 500 fsUSDCe into the system', async () => {
    assert.equal(await getCollateralBalance({ address, symbol: 'fsUSDCe' }), 1000);
    assert.deepEqual(await getAccountCollateral({ accountId, symbol: 'fsUSDCe' }), {
      totalDeposited: 0,
      totalAssigned: 0,
      totalLocked: 0,
    });

    await depositCollateral({ privateKey, symbol: 'fsUSDCe', accountId, amount: 500 });

    assert.equal(await getCollateralBalance({ address, symbol: 'fsUSDCe' }), 500);
    assert.deepEqual(await getAccountCollateral({ accountId, symbol: 'fsUSDCe' }), {
      totalDeposited: 500,
      totalAssigned: 0,
      totalLocked: 0,
  ...
  it('should delegate 500 fsUSDCe into the Spartan Council pool', async () => {
    assert.deepEqual(await getAccountCollateral({ accountId, symbol: 'fsUSDCe' }), {
      totalDeposited: 500,
      totalAssigned: 0,
      totalLocked: 0,
