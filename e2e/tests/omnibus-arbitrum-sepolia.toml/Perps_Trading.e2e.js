...
describe(require('path').basename(__filename, '.e2e.js'), function () {
  ...
  it('should set precompiled Arbitrum Gas Oracle', async () => {
    await provider.send('anvil_setCode', [
      '0x000000000000000000000000000000000000006C',
      require('./precompiled-gas-oracle.json'),
    ]);
    assert.ok('Precompiled Arbitrum Gas Oracle contract is set');
  });
  ...
  it('should set fBTC balance to 25', async () => {
    ...
    assert.equal(
      await getCollateralBalance({ address: wallet.address, symbol: 'fBTC' }),
      0,
      'New wallet has 0 fBTC balance'
    );
    await setMintableTokenBalance({ privateKey: wallet.privateKey, tokenAddress, balance: 25 });
    assert.equal(await getCollateralBalance({ address: wallet.address, symbol: 'fBTC' }), 25);
  });
  ...
  it('should approve sBTC spending for PerpsMarketProxy', async () => {
    assert.equal(
      await isCollateralApproved({
        address: wallet.address,
        symbol: 'sBTC',
  ...
  it('should make 5 sBTC deposit to Perps', async () => {
