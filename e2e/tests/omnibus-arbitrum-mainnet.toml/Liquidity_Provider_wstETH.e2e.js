...
const friendlyWhale = '0x513c7E3a9c69cA3e22550eF58AC1C0088e918FFf'; // Contract "Aave: aArbwstETH Token"
...
describe(require('path').basename(__filename, '.e2e.js'), function () {
  ...
  it(`should increase max pool collateral for the test`, async () => {
    const { tokenAddress } = await getCollateralConfig('wstETH');
  ...
  it('should set wstETH balance to 10', async () => {
    assert.equal(
      await getCollateralBalance({ address, symbol: 'wstETH' }),
      0,
      'New wallet has 0 wstETH balance'
    );
    const { tokenAddress } = await getCollateralConfig('wstETH');
    await setTokenBalance({ wallet, balance: 10, tokenAddress, friendlyWhale });
    assert.equal(await getCollateralBalance({ address, symbol: 'wstETH' }), 10);
  });

  it('should approve wstETH spending for CoreProxy', async () => {
    assert.equal(
      ...
      false,
      'New wallet has not allowed CoreProxy wstETH spending'
    );
  ...
  it('should deposit 5 wstETH into the system', async () => {
    assert.equal(await getCollateralBalance({ address, symbol: 'wstETH' }), 10);
    assert.deepEqual(await getAccountCollateral({ accountId, symbol: 'wstETH' }), {
      totalDeposited: 0,
      totalAssigned: 0,
      totalLocked: 0,
    });

    await depositCollateral({ privateKey, symbol: 'wstETH', accountId, amount: 5 });

    assert.equal(await getCollateralBalance({ address, symbol: 'wstETH' }), 5);
    assert.deepEqual(await getAccountCollateral({ accountId, symbol: 'wstETH' }), {
      totalDeposited: 5,
      totalAssigned: 0,
      totalLocked: 0,
  ...
  it('should delegate 5 wstETH into the Spartan Council pool', async () => {
    assert.deepEqual(await getAccountCollateral({ accountId, symbol: 'wstETH' }), {
      totalDeposited: 5,
      totalAssigned: 0,
      totalLocked: 0,
