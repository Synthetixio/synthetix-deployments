...
describe(require('path').basename(__filename, '.e2e.js'), function () {
  ...
  it('should have XRP market deployed among others', async () => {
  ...
  it('should have max open interest 1500000 XRP', async () => {
    const maxOpenInterest = parseFloat(
      ethers.utils.formatEther(await PerpsMarketProxy.maxOpenInterest(400))
    );
    log({ maxOpenInterest });
    assert.equal(maxOpenInterest, 1500000);
  });

  it('should get XRP market summary with ERC7412', async () => {
    await fulfillOracleQuery({ wallet, isTestnet: true, symbol: 'LTC' });
    const data = await PerpsMarketProxy.getMarketSummary(400);
    log({ data });
    const marketSummary = {
  ...
  it('XRP Market Funding Parameters have been correctly set', async () => {
    const { skewScale, maxFundingVelocity } = await PerpsMarketProxy.getFundingParameters(400);
  ...
  it('should have 1500000 XRP Max Market Size', async () => {
    const maxSize = await PerpsMarketProxy.getMaxMarketSize(400);

    assert.equal(ethers.utils.formatEther(maxSize), 1500000);
  });

  it('XRP Market should have 0.0003 Maker fee, 0.0007 Taker fee', async () => {
    const { makerFee, takerFee } = await PerpsMarketProxy.getOrderFees(400);

    assert.equal(Number(ethers.utils.formatEther(makerFee)), 0.0003);
    assert.equal(Number(ethers.utils.formatEther(takerFee)), 0.0007);
  });

  it('XRP Market Liquidation Parameters are correctly set', async () => {
    const {
    ...
    } = await PerpsMarketProxy.getLiquidationParameters(400);
  ...
  it('XRP Market Max liquidation parameters are correctly set', async () => {
    const {
      maxLiquidationLimitAccumulationMultiplier,
      maxSecondsInLiquidationWindow,
      maxLiquidationPd,
      endorsedLiquidator,
    } = await PerpsMarketProxy.getMaxLiquidationParameters(400);
  ...
  it('XRP Market Max locked OI ratio is correctly set', async () => {
    const maxLockedRatio = await PerpsMarketProxy.getLockedOiRatio(400);
